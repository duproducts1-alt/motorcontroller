...
int main(void)
{
  ...
  MC_StartMotor1();
  sprintf(buffer, "Motor started\r\n");
  HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);

  HAL_Delay(500);

  sprintf(buffer, "Initial alignment status: %d\r\n", MC_GetAlignmentStatusMotor1());
  HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);
  ...
  sprintf(buffer, "ALIGNMENT COMPLETE!\r\n");
   HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);

   MC_ProgramPositionCommandMotor1(1.0, 0.1);  // <-- ONCE, before loop
   ...
   /* USER CODE BEGIN 3 */
     ...
     float motor_pos = MC_GetCurrentPosition1();
     float target_pos = MC_GetTargetPosition1();
     int16_t encoder_raw = MC_GetElAngledppMotor1();
     PosCtrlStatus_t pos_status = MC_GetControlPositionStatusMotor1();

     int pos_int = (int)(motor_pos * 10000);
     int cmd_int = (int)(target_pos * 10000);

     sprintf(buffer, "CMD: %d | POS: %d | ENC: %d | STAT: %d\r\n", cmd_int, pos_int, encoder_raw, pos_status);
     HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 100);
